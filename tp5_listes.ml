...
let rec supprime liste v =
  match liste with 
  | [] -> []
  | hd::tl when hd=v -> supprime tl v 
  | hd::tl -> hd :: supprime tl v;;


let supprime liste v =
  let rec aux l va acc =
    match l with 
    | [] -> acc
    | hd::tl when hd=v -> aux tl va acc
    | hd::tl -> aux tl va (hd::acc) in 
  aux liste v [];;
